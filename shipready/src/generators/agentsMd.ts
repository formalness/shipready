...
export function buildCommandSection(project: ProjectInfo): string {
  const pm = project.packageManager;
  const s = project.scripts;
  const rows: string[] = [`- Install: \`${installCommand(pm)}\``];
  if (s.dev) rows.push(`- Dev: \`${runCommand(pm, "dev")}\``);
  if (s.build) rows.push(`- Build: \`${runCommand(pm, "build")}\``);
  if (s.test) rows.push(`- Test: \`${runCommand(pm, "test")}\``);
  if (s.lint) rows.push(`- Lint: \`${runCommand(pm, "lint")}\``);
  return rows.join("\n");
}

/** Builds a short project structure summary from scanned files. */
export function buildStructureSummary(project: ProjectInfo): string {
  const dirs = new Set<string>();
  for (const f of project.sourceFiles) {
    const top = f.split("/")[0];
    if (top && !top.startsWith(".") && f.includes("/")) dirs.add(top);
  }
  const list = [...dirs].sort().slice(0, 12);
  if (list.length === 0) return "- Flat project (no top-level source directories detected)";
