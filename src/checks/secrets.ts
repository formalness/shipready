...
const PLACEHOLDER_RE =
  /your[-_ ]?|example|placeholder|changeme|change[-_ ]me|dummy|fake|test[-_ ]?key|<[^>]*>|x{4,}|\*{4,}|123456|abcdef/i;
...
export function maskSecret(secret: string): string {
  ...
  const prefixMatch = secret.match(
    /^(sk-ant-|sk-proj-|sk-|sk_live_|rk_live_|ghp_|github_pat_|AIza|xoxb-|xoxp-|AKIA|sbp_|vercel_|npm_|SG\.|AC|SK|eyJ)/
  );
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const { kind, re } of PATTERNS) {
      ...
      if (match) {
        ...
        if (
          allowlist.some((a) => match[0].includes(a) || line.includes(a))
        ) {
          continue;
        }
        found.push({
